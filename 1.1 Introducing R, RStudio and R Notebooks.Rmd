...
This has not been edited or even checked -- it's just the same as what was created for Spring 2019 as homework before the first digital lab. I'm likely to do that again, having it assigned and have some sort of assignment inside of it, so that students have to work through to the end, do a small exercise, and then turn it in, maybe on Canvas and maybe on GitHub. People will turn it in 1.5 weeks before the first digital lab. Accordingly, during the lab the week before the first digital lab, if people are really struggling there can be a help session at some point (mid-way break in lab, or at end). Then I will know everyone has done it and can move a bit more quickly and confidently in first in-person digital lab. 

# Edits:
# Line 65
# Line 273, see if this error still happens after restarting R
# Line 348, same question as Line 65

### Start

Welcome to your first experience (in this class) with an [R Markdown](http://rmarkdown.rstudio.com) Notebook. 

R is a programming language designed for using data: compiling, visualizing, analyzing, testing statistically.
...
####2. It is popular and democratic, partly because it is free and open source. It was developed by people who were donating their labor, or people whose employers consider their contributions to R as part of their work, such as professors and other scholars and coders. 
...
Wasn't that fun? The reason that worked so well is that RStudio has a "demo" dataset loaded whenever it loads what is referred to as "base R." 
...
This Notebook should be read and used before "Introduction to getting, testing and plotting data." Definitely follow your work with this Notebook by opening and doing that companion lesson. 
# Will you have this additional Notebook for them?
...
I'm also grateful to my student assistants Jared Meek (Columbia E3B student) and Rhea Nagpal (Barnard student fellow at the Empirical Reasoning Center).
...
Below there is a series of commands that you can run in one of two ways; try both!

1. Simply use your cursor to highlight the line of code, then press *Ctrl+Enter* 
2. Press the Insert button in the menu above this window (or press *Ctrl+Alt+I*), then copy/paste (or re-type or write) your code in the middle of the blank chunk that's been opened. Run the chunk by pressing the play button in the upper right corner of the chunk.
...
4a. Try running the chunk below. Just press the "play" icon in the upper right corner. 
...
```{r}
xlist = c(1,2,3,5)
xlist #This should re-print the vector just defined
class(xlist) #This should tell you that this is a numeric vector
length(xlist) #This should report the length of the vector, which means how many values it contains
```
...
#### Try count, which should report the frequency of each variable in the list
count(zlist)
# Could not find function "count"??

#### Or unique, which will list each of the values in the vector
unique(zlist)

#### To illustrate this a little better, try this
zzlist = c("a", "b", "c", "q", "a", "b", "c", "q")
length(zzlist)
count(zzlist)
unique(zzlist)
...
# Again, will we have this follow up lesson?
=======
