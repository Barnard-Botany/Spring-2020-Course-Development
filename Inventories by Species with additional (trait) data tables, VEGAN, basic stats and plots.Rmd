...
This is a monster of a notebook. One thing to look out for is overly repetitious places, and maybe opportunities to break it into smaller pieces. 
...
Now we have output. That came from the last line, line 28. 

What does this output mean? It is printing our sample, as a vector variable. Just by looking at how we typed in the variables, or at the output, it's possible to see that Species A, Species B and Species C are all equally common. In contrast, Species D, E, F, G, and H are rare in this sample. Each appears only once, known as "singletons".
...
We will now be starting to move beyond just entering data into R and looking at it. Now we need to add three packages to your R Studio software, and then load those packages. This is another super-important lesson that you'll adapt for repeated use.
...
#Let's also re-name columns. Later, our analyses of species richness will need specifically named variables. 
...
#That means we must load this package. (You may need to install it before loading it, if you haven't already done so).
#library("labdsv")
...
dim(onethree.ab)
```
The command "dim" refers to the dimensions of this data frame: it has two rows and nine columns. 
...
Let's start by defining one. It's the Shannon-Weaver Diversity Index (H). This may be familiar if you have taken ecology or conservation biology. 
...
D       1   1/23 = 0.043	  -3.13    	-0.136
E       1   1/23 = 0.043	  -3.13    	-0.136
F       1   1/23 = 0.043	  -3.13    	-0.136
G       1   1/23 = 0.043	  -3.13    	-0.136
H       1   1/23 = 0.043	  -3.13    	-0.136
Q       0   0/23 = 0          0          0
				H = -(-1.73)
...
Another measure of diversity is Simpson's index. There are actually a lot of different versions of Simpson's index, all roughly based on the likelihood that any two randomly encountered individuals are the same species. 
...
# For both Shannon (H) and Simpson (D) indices, higher values indicate more diversity.

# Using R to check your manual calculations of H, D
...
```{r}
#First let's make a vector with a list of trees on Cumming Street, which is in Inwood, a Manhattan neighborhood.
cumming <- c("Ginkgo biloba",
"Acer rubrum",
"Ginkgo biloba",
"Pyrus calleryana",
"Tilia americana",
...
length(cumming)
length(West217)
#The command unique will count the number of species on the block
unique(cumming)
unique(West217)
#you can combine the two into one step
length(unique(cumming))
...
cumming.ab <- count(cumming)
cumming.ab
West217.ab <- count(West217)
West217.ab

#Now add ID column and rename columns
cumming.ab <- data.frame("cumming", cumming.ab)
names(cumming.ab) <- c("ListID", "SpeciesID", "Freq")
cumming.ab
...
shannonC <- diversity(cumming.ab$Freq, index = "shannon")
shannonC
shannonW <- diversity(West217.ab$Freq, index = "shannon")
shannonW
simpsonC <- diversity(cumming.ab$Freq, index = "simpson")
simpsonC
...
inwood <- rbind(cumming.ab, West217.ab)
inwood
...
# We should include a simple summary statement that explains what high and low values indicate for all four indices!!!
# EX: For Bray-Curtis, values closer to 0 indicate more similarity between pairwise samples; for Jaccard's Coefficient, values closer to 1 indicate more similarity.
...
### What is a Multi-dimensional Ordination? 

```{r}
head(abundance)
example_NMDS <- metaMDS(abundance, k=2) # Our community-by-species matrix 
# K=2 is setting the number of reduced dimensions. Increase if high stress is a problem: "The stress, or the disagreement between 2-D configuration and predicted values from the regression"
...
If you again exit (temporarily) from R Studio, you could go to other software (Excel, Google Sheets) and open the file there. (Feel free to try that.) 
...
ALLtrees <- fn
#The file we just imported is a "data frame"; we can use the class() function to learn what R calls our imported files
class(ALLtrees)
...
dim(Strees)
#We use the command sub-set and ask to get only Brooklyn trees, and to exclude trees where there is missing data for species name (which happens because trees are stumps or dead)
...
Brooklyn.ab
#Now you can get things like species richness, which is the number of species
#That's sort of silly, because we know there are 132 species from the command "length"
...
For practice, go through this code and modify it so that you can get data for another borough, or even for all the boroughs. 

```{r}
#But there is a way to somewhat streamline this, which we were actually using with the toy example at the very beginning. So let's bring that back.
#Get a frequency table from the full subset of Brooklyn street tree data, as before
Brooklynfreq <- count(Brooklyn$spc_common)
#We are not going to transpose
#Instead, we are going to add an initial left-hand column to the frequency table identifying it as the data from Brooklyn and give three super-specific column names to the 3 columns
...
jaccard
```
# Remember, for Bray-Curtis, values closer to 0 indicate more similarity between pairwise samples; for Jaccard's Coefficient, values closer to 1 indicate more similarity.

# Which boroughs are most similar in plant diversity? Least diversity? Do the two indices used here agree with each other? Why or why not?

```{r}
#Now that there are five samples, it makes sense to start seeing if this is enough samples to gauge diversity across these samples. Vegan can compute the cumulative number of species across all five, and then plot them. It will take the plots in random order. Other options for this method argument include "collector" which is the order in which they are entered into the matrix. Another is "rarefaction" which will adjust to take into account that each sample may have a different number of individuals.
...
### Multi-dimensional ordination, or multi-dimensional scaling (MDS), is a set of techniques used to visualize the similarity of individuals in a dataset.
...
## This code isn't working and I can't figure out why; are we wanting to run species accumulation curves for the produce data? I'm skipping 709-745 for now.
```{r}
#We will repeat the plotting functions used for NYC Trees above and use them on this NYC produce data frame.
accurve.prod <- specaccum(produce, method="random", permutations=300)
plot(accurve.prod$sites, accurve.prod$richness,
     xlab="Number of Sites",
     ylim=c(0, 200),
     ylab="Species Richness",
     main="NYC Produce")
accurve2.prod <- specaccum(produce, method="rarefaction", permutations=50)
plot(accurve2.prod$sites, accurve2.prod$richness,
...
plot(tree_dbh~spc_common, data=ALLtrees, outline=T)
plot(tree_dbh~spc_common, data=ALLtrees, outline=F)
plot(tree_dbh~spc_common, data=ALLtrees, outline=F, horizontal=T)

##It's difficult to read the x-axis within RStudio when there are this many tree species, but here is a solution:
plot(tree_dbh~spc_common, data=ALLtrees, outline=F, las=2)

#The 'las=2' command specifies that you want your labels perpendicular to the axis.
#If your plot isn't showing up in the 'Plots' display of RStudio, then copy, paste and run the code on line 768 directly into your console.
#When your plot appears, click the 'Export' dropdown menu and choose 'Save as Image'.
#In the top right of your save options, set Width=2500 and Height=1000, then save the image to your Desktop and observe the widened plot.
```

# Is this a good place to split this notebook?

# STATISTICS

Let's import some data. Francis Galton, one of the founders of statistics, also promoted the science of eugenics. (For a long and wonderful feminist discussion about eugenics, see the book Ghost Stories for Darwin by Banu Subramanian). Galton was also the cousin of Charles Darwin. Galton invented the term "regression". These days, "regression" means fitting the best-fit line to a series of x and y data points. But, why is the weird term "regression" used for this? What is regressing?

Let's look at Galton's original dataset: the heights of parents and children. Source for this data can be found [here](http://www.randomservices.org/random/data/Galton.html).

## TESTING DATA

```{r}
#Import "Galton.txt" data set as data frame named 'heights'
heights <- read.table("Galton.txt", header=TRUE, sep="\t")
#And, look at 'heights'
heights
```
...
You can "comment out" lines to be sure they won't run as a command. For heights, add a # in front of it.
...
```{r}
# To summarize, these simple functions are great for quickly viewing information about objects in R. I encourage you to memorize them.
# View top 6 rows of object
head(heights)
# Column names of object
names(heights)
...
```{r}
# Remember, you can check the class of each variable in your data frame by running:
str(heights)
# Here you see that the 'Family' and 'Gender' variables are in a "Factor" format (non-numeric), which is why they don't work in numeric-based functions.
```

Comment out that very first line (line 834) by adding "#" at the front of the line.

What line 835 is asking for is to use the same dataframe, but to skip the first column and the fourth column.

Now run the chunk again.

Here, the "Height" variable refers to the kids' heights. Let's make this more obvious.
```{r}
# Change "Height" variable to the name "Child"
colnames(heights)[colnames(heights)=="Height"] <- "Child"
```
...
heights$Child = as.numeric(heights$Child)
...
ttest_result1 = t.test(x=heights$Midparent, y=heights$Child, paired=FALSE, alternative="two.sided")
ttest_result1
```
...
ttest_result2 = t.test(x=heights$Midparent, y=heights$Child, paired=TRUE, alternative="two.sided")
ttest_result2
```
...
You can also run code in an R Notebook by highlighting line(s) and pressing the "Run" icon above, or by pressing Control+Enter. But, the code will show up in the Console window (below). If you want the output to be incorporated into your R Notebook, then put the code into a chunk and then run the chunk. Output will not only appear in this window, but will also be saved into an accompanying notebook html file if you save your work before exiting.
...
# Plot the mean
abline(v=mean(heights$Child), lty="dashed", lwd=2, col="blue")

# Plot the 95% confidence interval (2.5% - 97.5%)
CI_025 <- mean(heights$Child) - 1.96*sd(heights$Child)
CI_975 <- mean(heights$Child) + 1.96*sd(heights$Child)
abline(v=CI_025, lty="dotted", lwd=2, col="blue")
abline(v=CI_975, lty="dotted", lwd=2, col="blue")
...
# Child values
hist(heights$Child, xlim=xlims, ylim=ylims, xlab="height (inches)", ylab="Count", main="Child heights")
...
Ftest_result <- var.test(x=heights$Midparent, y=heights$Child, ratio=1, alternative="two.sided")
Ftest_result
```
...
======
For the complex story of Galton's original data, [go here](http://www.medicine.mcgill.ca/epidemiology/hanley/galton/).
...
QUESTION: is there a relationship between parent and child height?

Start by plotting the data:

```{r}
plot(x=heights$Midparent, y=heights$Child)
...
plot(x=jitter(heights$Midparent), y=jitter(heights$Child))
```
...
lm_result <- lm(formula=Child~Midparent, data=heights)
lm_result
...
xlims <- c(5*12, 6.5*12)
ylims <- c(5*12, 6.5*12)
plot(x=jitter(heights$Midparent, factor=3), y=jitter(heights$Child, factor=3), xlab="Midparent height", ylab="Child height", xlim=xlims, ylim=ylims)
...
child_minus_1to1 <- heights$Child - (1/1*heights$Midparent)
heights2 <- heights
...
# Make sure to delete all of these edits and the very first "Jared" section when finished.
# Line 37 may need to be updated w/ correct line number when notebook is done.
# Line 205, Added explanation of H and D
# Line 400 and 401 summary statement: need something short and sweet, similar to line 205
# Line 550, what is NYCspecies=spec trying to do, along with Line 551
# Added 652 and 654 description and questions...are these instructive?
# The Produce diversity section is confusing; not sure what direction we want to take this. Section skipped starting at 708; go back and look over this with Hilary.
# Added some stuff at 767 to help visualize ALLtrees DBH boxplots better.
# 776, consider splitting notebook into two here.